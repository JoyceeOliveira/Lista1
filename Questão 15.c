...
// >>>>>> Código para ordenação triangular/árvore <<<<<< //

void faz_triangulacao (float *vetor, int i, int f){
    ...
    while(j <= f){ // enquanto o valor final for maior ou iagual ao primeiro ramo
        if(j<f){   // quando o primeiro ramo for menor que o ramo final
...
void triangular (float *vetor, int n){
    ...
    for(i = n-1; i>=1; i--){
        ...
        faz_triangulacao(vetor, 0, i-1); // Refazendo a triangulação
    }
}
